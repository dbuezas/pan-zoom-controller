...
export class DigitalPTZ {
  ...
  private onTouchStart = async (e: TouchEvent) => {
    ...
    let isValid = true;
    const isQuickTap =
      e.timeStamp - this.lastTap < ONE_FINGER_DOUBLE_TAP_ZOOM_MS_THRESHOLD;
    ...
    if (
      ...
      if (this.transform.scale == 1) {
        this.transform.zoomAtCoords(
          2,
          this.lastTouches[0].clientX,
          this.lastTouches[0].clientY
        );
      } else {
  ...
  private onWheel = async (e: WheelEvent) => {
  ...
  private onMouseDown = async (e: MouseEvent) => {
