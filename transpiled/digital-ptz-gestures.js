...
function startDoubleTapZoom({ containerEl, transform, render }) {
    ...
    const onTouchStart = (downEvent) => {
        ...
        const onTouchEnd = (endEvent) => {
            ...
            const didMove = 30 <
                Math.hypot(endEvent.changedTouches[0].clientX - downEvent.touches[0].clientX, endEvent.changedTouches[0].clientY - downEvent.touches[0].clientY);
            if (!isQuickRelease || didMove)
                return;
            const zoom = transform.scale == 1 ? 2 : 0.01;
            transform.zoomAtCoords(zoom, downEvent.touches[0].clientX, downEvent.touches[0].clientY);
            render();
        };
...
function startOneFingerPan({ containerEl, transform, render }) {
    const onTouchStart = (downEvent) => {
        ...
        const onTouchEnd = () => containerEl.removeEventListener("touchmove", onTouchMove);
        containerEl.addEventListener("touchend", onTouchEnd, { once: true });
    };
    containerEl.addEventListener("touchstart", onTouchStart);
    return () => containerEl.removeEventListener("touchstart", onTouchStart);
...
function startPinchZoom({ containerEl, transform, render }) {
    const onTouchStart = (downEvent) => {
        ...
        const onTouchMove = (moveEvent) => {
            ...
            const oldSpread = Math.hypot(lastTouches[0].clientX - lastTouches[1].clientX, lastTouches[0].clientY - lastTouches[1].clientY);
            const newSpread = Math.hypot(newTouches[0].clientX - newTouches[1].clientX, newTouches[0].clientY - newTouches[1].clientY);
...
function startDoubleClickZoom({ containerEl, transform, render, }) {
    ...
    const onMouseDown = (downEvent) => {
        ...
        if (!isSecondClick)
            return;
        capture(downEvent);
        containerEl.addEventListener("mouseup", (upEvent) => {
...
export { startDoubleTapZoom, startOneFingerPan, startPinchZoom, startTouchTapDragZoom, startMouseWheel, startDoubleClickZoom, startMouseDragPan, };
