...
import { startMouseWheel, startDoubleClickZoom, startDoubleTapZoom, startMouseDragPan, startOneFingerPan, startPinchZoom, startTouchTapDragZoom, } from "./digital-ptz-gestures.js";
...
export class DigitalPTZ {
    ...
    constructor(containerEl, videoEl, options) {
        ...
        if (o.mouse_drag_pan)
            h.push(startMouseDragPan(gestureParam));
        if (o.mouse_wheel_zoom)
            h.push(startMouseWheel(gestureParam));
        if (o.mouse_double_click_zoom)
            h.push(startDoubleClickZoom(gestureParam));
        if (o.touch_double_tap_zoom)
            h.push(startDoubleTapZoom(gestureParam));
        if (o.touch_tap_drag_zoom)
            h.push(startTouchTapDragZoom(gestureParam));
        if (o.touch_drag_pan)
            h.push(startOneFingerPan(gestureParam));
        if (o.touch_pinch_zoom)
            h.push(startPinchZoom(gestureParam));
    ...
    destroy() {
        for (const off of this.offHandles)
            off();
